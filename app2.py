...
TASA_ROTACION_CARTERA_DIAS = 30 # Días de rotación de cartera
HORIZONTE_PROYECCION_DEFAULT = 12 # Meses de proyección por defecto
NUM_SIMULACIONES_DEFAULT = 5000 # Número de simulaciones de Monte Carlo

# Factores Ambientales (Para Huella de Carbono)
EF_COMBUSTIBLE = 2.68  # Kg CO2e/L
EF_ELECTRICO = 0.20    # Kg CO2e/kWh
DIAS_TRABAJO_ANUAL = 240 # Para cálculo de Indicadores HSEQ

MODULOS = [
    "Dashboard General",
    "Módulo Gobernanza y Dueños",
    "Módulo Financiero",
    "Módulo Sostenibilidad Social RRHH",
    "Módulo Sostenibilidad Ambiental",
    "Módulo Riesgos HSEQ",
    "Módulo Comercial",
    "Módulo Operativo (Productividad)"
]

# ==============================================================================
# 2. AUTENTICACIÓN Y SEGURIDAD (Simulación)
# ==============================================================================
...
def check_password():
    ...
    if "password_correct" not in st.session_state:
        # Primera ejecución, muestra la entrada de contraseña.
        st.title("🌊 Horizonte Empresarial 360")
...
def load_data_from_upload(uploaded_files: List[Any]) -> Dict[str, pd.DataFrame]:
    ...
    # Mapeo de nombre de hoja a variable de sesión y nombre de columna de FECHA
    ...
    for file in uploaded_files:
        try:
            ...
            for sheet_name in xls.sheet_names:
                map_info = None
                # Buscar la información de mapeo para el nombre de la hoja (insensible a mayúsculas/minúsculas)
                ...
                if map_info is None:
                    st.sidebar.info(f"⏭️ Saltando hoja '{sheet_name}' en '{file.name}' (no reconocida o no necesaria).")
                    continue
                ...
                # Renombrar columna de fecha a un estándar 'FECHA' para consistencia
                ...
                st.sidebar.success(f"✅ Cargado: {map_info['var'].replace('df_', '').upper()} (desde hoja '{sheet_name}' de '{file.name}')")

        except Exception as e:
            st.sidebar.error(f"❌ Error al cargar/procesar el archivo '{file.name}' o alguna de sus hojas: {e}")

    # Verificar que todos los DataFrames requeridos estén presentes
    ...
    for df_name in required_dfs:
        if df_name not in loaded_data:
            loaded_data[df_name] = pd.DataFrame() # DataFrame vacío si falta

    return loaded_data


# ==============================================================================
# 4. FUNCIONES DE CÁLCULO Y VISUALIZACIÓN POR MÓDULO
# ==============================================================================

# --- Módulo de Gobernanza y Dueños ---
def display_gobernanza_module(all_dfs: Dict[str, pd.DataFrame]):
    st.title("👑 Módulo Gobernanza y Dueños")
    st.subheader("Visión Estratégica y Health Score")
    ...
    st.markdown("""
        Este módulo consolida el rendimiento de la organización en un *Health Score* integral para la toma de decisiones al más alto nivel.
    """)
    ...
    # Bandera para verificar si todos los datos mínimos están presentes para el Health Score
    ...
    if not df_financiero.empty and 'FECHA' in df_financiero.columns and \
       ...
       not df_rrhh.empty and 'Tasa_Rotacion_Anual' in df_rrhh.columns:
        ...
        # Indicador Social (Rotación)
        avg_rotacion = df_rrhh['Tasa_Rotacion_Anual'].mean() / 100 # Promedio histórico

        # Indicador Riesgos (Índice de Gravedad)
        ...
        # Simulación de Health Score (0 a 100)
        ...
        score_social = min(100, max(0, (1 - (avg_rotacion / 0.18)) * 100)) # Basado en 18% de rotación como máximo
        score_riesgos = min(100, max(0, (1 - (indice_gravedad / 50)) * 100)) # Basado en 50 como índice de gravedad máxima

        health_score = (score_financiero * 0.35 + score_ambiental * 0.25 + score_social * 0.20 + score_riesgos * 0.20)

    else:
        st.warning("No hay datos cargados para generar el Health Score o faltan DataFrames/columnas clave (Financiero, Sostenibilidad, Riesgos, RRHH). Asegúrese de cargar todos los archivos requeridos.")
        can_calculate_health_score = False

    st.subheader("Indicadores Clave Consolidados (Último Periodo)")
    st.caption(f"Datos consolidados hasta: {latest_date_str}")

    col1, col2, col3, col4, col5 = st.columns(5)

    col1.metric("Ingresos Último Mes", f"COP {ingresos:,.0f}", delta_color="normal")
    ...
    col4.metric("Tasa de Rotación Anual (Promedio)", f"{avg_rotacion*100:.2f}%", delta_color="inverse")
    col5.metric("Índice de Gravedad HSEQ", f"{indice_gravedad:.2f}", delta_color="inverse")

    st.markdown("---")
    st.subheader("🎯 Health Score Empresarial 360")
    st.metric("Health Score General", f"{health_score:.1f}/100",
              help="Índice ponderado basado en el rendimiento financiero, ambiental, social y de riesgos.")

    if can_calculate_health_score: # Solo mostrar el gráfico si se pudo calcular el score
...
def display_financiero_module(df_financiero: pd.DataFrame, HORIZONTE_PROYECCION: int, NUM_SIMULACIONES: int):
    st.title("💰 Módulo Financiero: Gestión de Liquidez y Cash Flow")
    ...
    st.markdown("---")
    with st.expander("⚙️ Configuración de Escenarios de Proyección"):
        st.info("La proyección usa la media histórica y la desviación estándar de los flujos de caja (Cobros y Pagos) para simular el riesgo (Método Monte Carlo).")
        st.caption("Ajuste los parámetros del simulador:")

        # Inputs para el usuario
        col1, col2 = st.columns(2)
        with col1:
            cobros_ajuste = st.number_input("Ajuste de Cobros (% sobre media histórica)", min_value=-50, max_value=50, value=0, help="Ajusta la media de Cobros. Ej: 5 para un escenario optimista (+5%).")
        with col2:
            pagos_ajuste = st.number_input("Ajuste de Pagos (% sobre media histórica)", min_value=-50, max_value=50, value=0, help="Ajusta la media de Pagos. Ej: -5 para un escenario optimista en costos (-5%).")

    # 2. Análisis Histórico de Flujos
    ...
    st.subheader("Análisis Histórico de Efectivo")
    ...
    if df_financiero.shape[0] >= 2 and 'Efectivo_Final' in df_financiero.columns:
        efectivo_final_ultimo_periodo = df_financiero['Efectivo_Final'].iloc[-2] # El penúltimo es el último 'final' antes de la proyección
    elif df_financiero.shape[0] == 1 and 'Efectivo_Final' in df_financiero.columns: # Si solo hay un registro, ese es el último
    ...
    col_hist2.metric("Efectivo Final (Último Periodo)", f"COP {efectivo_final_ultimo_periodo:,.0f}")
    ...
    if not df_financiero.empty:
        fig_hist = px.line(df_financiero.tail(24), x='FECHA', y=['Cobros_Clientes', 'Pagos_Proveedores', 'Gasto_Nomina'],
                           title='Tendencia Histórica de Flujos de Caja (Últimos 2 Años)',
        ...
        st.info("No hay suficientes datos históricos para mostrar la tendencia de flujos de caja.")

    # 3. Simulación de Monte Carlo (Flujo de Caja Proyectado)
    ...
    # Ajuste de la media según el input del usuario
    ...
    effective_start_cash = 0.0 # Reinicializar para la simulación
    ...
    st.subheader(f"Simulación de {NUM_SIMULACIONES} Escenarios a {HORIZONTE_PROYECCION} Meses")
    ...
    if df_financiero.shape[0] > 0 and 'Efectivo_Final' in df_financiero.columns:
        try:
            ...
            # Creación de fechas de proyección
            ...
            fig_proj.add_trace(go.Scatter(x=df_sim.index, y=df_sim['Media'], mode='lines',
                                          line=dict(color='blue', width=2), name='Proyección Media'))

            # Umbral de Liquidez Mínima
            ...
            fig_proj.update_layout(title=f'Proyección de Efectivo Final a {HORIZONTE_PROYECCION} Meses',
                                   yaxis_title='Efectivo (COP)', xaxis_title='Mes de Proyección',
                                   hovermode="x unified")
            st.plotly_chart(fig_proj, use_container_width=True)

            # 4. Conclusión de Riesgo
            risk_months = df_sim[df_sim['P5'] <= UMBRAL_LIQUIDEZ_MINIMA].shape[0]
            if risk_months > 0:
                st.error(f"🚨 ALERTA DE RIESGO: El Escenario Pesimista (P5) predice que el Efectivo Final caerá por debajo del umbral de liquidez mínima (COP {UMBRAL_LIQUIDEZ_MINIMA:,.0f}) en {risk_months} de los {HORIZONTE_PROYECCION} meses proyectados. Se recomienda aplicar estrategias de gestión de capital de trabajo y cobranza.")
            else:
                st.success("✅ La proyección de liquidez es saludable. El Escenario Pesimista (P5) se mantiene por encima del umbral de alerta.")

        except Exception as e:
            st.error(f"Error al ejecutar la simulación de Monte Carlo: {e}")
            st.info("Asegúrese de que las columnas `Cobros_Clientes`, `Pagos_Proveedores`, y `Gasto_Nomina` sean numéricas y no contengan valores nulos significativos.")
    else:
        st.info("No hay datos financieros suficientes para realizar la simulación de Monte Carlo.")

    # 5. Descarga de Reporte
    st.markdown("---")
    def create_pdf_report(df: pd.DataFrame) -> BytesIO:
        """Simulación de generación de reporte PDF (usa CSV como placeholder)."""
        output = BytesIO()
        # En una aplicación real, usarías ReportLab o FPDF para generar un PDF.
        # Aquí, generamos el DF de simulación como CSV para la descarga.
    ...
    if not df_sim.empty and all(col in df_sim.columns for col in ['Media', 'P5', 'P95']): # Check if df_sim was successfully created and has expected columns
        ...
        st.download_button(
            label="Descargar Reporte de Proyección Financiera (CSV)",
            data=report_buffer,
            file_name="Reporte_Proyeccion_Cash_Flow.csv",
            mime="text/csv"
        )
    else:
        st.info("No hay datos de simulación disponibles para descargar el reporte.")

# --- Módulo de Sostenibilidad Social RRHH ---
def display_rrhh_module(df_rrhh: pd.DataFrame):
    st.title("👨‍👩‍👧‍👦 Módulo Sostenibilidad Social y RRHH")
    ...
    col1.metric("FTEs Promedio (Último Periodo)", f"{latest_data['FTEs_Promedio']:.1f}")
    col2.metric("Tasa de Rotación Anual", f"{latest_data['Tasa_Rotacion_Anual']:.2f}%", delta_color="inverse")
    col3.metric("Costo por FTE (Último Periodo)", f"COP {latest_data['Costo_x_FTE']:,.0f}")
    col4.metric("Inversión en Capacitación (Acumulada)", f"COP {df_rrhh['Inversion_Capacitacion'].sum():,.0f}")

    # 2. Análisis de Tendencia
    st.subheader("Tendencia de Rotación vs. Inversión en Capacitación")

    fig1 = px.line(df_rrhh.sort_values('FECHA'), x='FECHA', y='Tasa_Rotacion_Anual',
                   title='Tasa de Rotación Anual por Mes',
                   labels={'Tasa_Rotacion_Anual': 'Tasa de Rotación (%)', 'FECHA': 'Fecha'})
    st.plotly_chart(fig1, use_container_width=True)

    fig2 = px.bar(df_rrhh.sort_values('FECHA'), x='FECHA', y='Inversion_Capacitacion',
                  title='Inversión Mensual en Capacitación',
                  labels={'Inversion_Capacitacion': 'Inversión (COP)', 'FECHA': 'Fecha'})
    st.plotly_chart(fig2, use_container_width=True)

    # 3. Distribución de Costo por FTE
    st.subheader("Distribución de Costo Total de Nómina")

    fig3 = px.histogram(df_rrhh, x='Costo_x_FTE', nbins=20,
                        title='Distribución de Costo por FTE a lo largo del tiempo',
                        labels={'Costo_x_FTE': 'Costo por FTE (COP)'})
    st.plotly_chart(fig3, use_container_width=True)


# --- Módulo de Sostenibilidad Ambiental ---
def display_sostenibilidad_ambiental_module(df_sostenibilidad: pd.DataFrame):
    st.title("🌳 Módulo Sostenibilidad Ambiental")
    ...
    # 1. Indicadores Clave
    st.subheader("Métricas de Impacto Ambiental")
    ...
    col3.metric("Residuos Sólidos (Kg)", f"{total_residuos:,.0f} Kg", delta_color="inverse")
    col4.metric("Costo Acumulado Multas", f"COP {total_multa:,.0f}", delta_color="inverse")

    # 2. Huella de Carbono por Alcance (Gráfico de torta/sol)
    ...
    fig_co2 = px.pie(co2_alcance, values='CO2e', names='Alcance',
                     title='Distribución de Huella de Carbono (Kg CO2e)',
                     color_discrete_sequence=px.colors.sequential.Agsunset)
    st.plotly_chart(fig_co2, use_container_width=True)

    # 3. Tendencia de Consumos Críticos
    ...
    st.subheader("Costo de Multas y Número de Incumplimientos Vencidos")
...
def display_riesgos_hseq_module(df_riesgos: pd.DataFrame):
    st.title("🚨 Módulo Riesgos HSEQ (Salud, Seguridad, Ambiente y Calidad)")
    ...
    # Cálculos HSEQ Clásicos (Tasa de Frecuencia y Severidad)
    ...
    # Tasa de Severidad (IS) = (Días Perdidos / Horas Trabajadas) * 240,000
    indice_severidad = (total_dias_perdidos / total_horas) * 240000 if total_horas > 0 else 0

    # 1. Indicadores Clave de Riesgo
    st.subheader("Índices de Seguridad y Salud en el Trabajo")

    col1, col2, col3, col4 = st.columns(4)
    col1.metric("Total Incidentes (Accidentes)", f"{total_incidentes:,}")
    col2.metric("Índice de Frecuencia (IF)", f"{indice_frecuencia:.2f}", help="Incidentes por 240,000 horas hombre")
    col3.metric("Índice de Severidad (IS)", f"{indice_severidad:.2f}", delta_color="inverse", help="Días perdidos por 240,000 horas hombre")
    ...
    st.subheader("Distribución de Eventos de Riesgo")
    ...
    fig_event = px.bar(event_counts, x='Tipo_Evento', y='Conteo',
                       ...
                       labels={'Conteo': 'Número de Eventos', 'Tipo_Evento': 'Tipo de Evento'},
...
def display_comercial_module(df_comercial: pd.DataFrame):
    st.title("📈 Módulo Comercial: Gestión de Ventas y Margen")
    ...
    # 1. Indicadores Clave
    st.subheader("Métricas de Venta y Rentabilidad")
...
# --- Módulo Operativo (Productividad/Procesos) ---
def display_operativo_module(df_operativo: pd.DataFrame):
    st.title("🏭 Módulo Operativo (Productividad y Procesos)")
    ...
    # 1. Indicadores Clave
    st.subheader("Métricas de Eficiencia de Producción")
    ...
    col1, col2, col3, col4 = st.columns(4)
    col1.metric("OEE Promedio (Teórico)", f"{avg_oee:.2f}%", delta_color="normal", help="Eficiencia General del Equipo - Simplificado.")
    ...
    df_operativo['Mes_Año'] = df_operativo['FECHA'].dt.to_period('M').astype(str)

    fig1 = go.Figure()
    fig1.add_trace(go.Scatter(x=df_operativo['Mes_Año'], y=df_operativo['OEE_Teorico'] * 100, mode='lines+markers', name='OEE (%)', yaxis='y1'))
    fig1.add_trace(go.Scatter(x=df_operativo['Mes_Año'], y=df_operativo['Tasa_Defectos'] * 100, mode='lines+markers', name='Tasa de Defectos (%)', yaxis='y2', line=dict(color='red')))
    ...
    fig2 = px.scatter(df_operativo, x='Unidades_Defectuosas', y='Costo_Total_Reproceso',
                      ...
                      title='Relación entre Defectos y Costo de Reproceso (Tamaño: Costo Mano de Obra)')
    st.plotly_chart(fig2, use_container_width=True)

# --- Dashboard General ---
def display_dashboard(all_dfs: Dict[str, pd.DataFrame]):
    st.title("🌐 Dashboard General: Visión 360")
    st.subheader("Resumen de Métricas Clave de la Plataforma")

    st.markdown("""
        Este dashboard ofrece una perspectiva de alto nivel del rendimiento de **Acuamar S.A.**,
        consolidando los KPIs más críticos de los 7 módulos.
    """)
    ...
    col1, col2, col3, col4 = st.columns(4)
    col1.metric("💰 Margen Neto Operativo", f"COP {metrics.get('Margen_Neto_Op', 0):,.0f}", help="Ingresos - Egresos Operativos")
    col2.metric("📈 Margen Bruto Comercial (%)", f"{metrics.get('Margen_Bruto_Pct', 0):.2f}%")
    col3.metric("🏭 OEE Promedio", f"{metrics.get('OEE_Promedio', 0):.2f}%", delta_color="normal")
    col4.metric("👨‍👩‍👧‍👦 Rotación Anual (Últ. Mes)", f"{metrics.get('Rotacion_Anual', 0):.2f}%", delta_color="inverse")

    # 2. Row de Indicadores de Riesgo y Sostenibilidad
    st.header("2. Riesgos y Sostenibilidad")
    col5, col6, col7, col8 = st.columns(4)
    col5.metric("🚨 Índice Frecuencia HSEQ (IF)", f"{metrics.get('IF_HSEQ', 0):.2f}", delta_color="inverse")
    col6.metric("🌳 Huella CO2e (Acumulada)", f"{metrics.get('Huella_CO2e_Total', 0):,.0f} Kg", delta_color="inverse")
    col7.metric("💧 Costo Multas Ambientales", f"COP {metrics.get('Costo_Multas', 0):,.0f}", delta_color="inverse")
    col8.metric("📉 Tasa Promedio de Defectos", f"{metrics.get('Tasa_Defectos', 0):.2f}%", delta_color="inverse")

    # 3. Gráfico de Tendencia Integrada
    ...
    if not df_financiero.empty and 'FECHA' in df_financiero.columns and 'Flujo_Neto' in df_financiero.columns and \
       not df_operativo.empty and 'FECHA' in df_operativo.columns and 'OEE_Teorico' in df_operativo.columns:
        # Preparar datos para gráfico de línea dual
        ...
        if not df_merge.empty:
            ...
            fig_trend.update_layout(
                title='Relación: Flujo Neto (Finanzas) vs. OEE (Operaciones)',
                xaxis_title='Fecha',
...
if check_password():
    ...
    # Carga de archivos al inicio de la sesión
    ...
    with st.sidebar.expander("📂 Cargar Datos"):
        st.caption("Cargue todos los archivos .xlsx requeridos. Puede subir varios archivos Excel, y el sistema buscará las hojas correspondientes por nombre (financiero, comercial, rrhh_social, sostenibilidad, riesgos_hseq, productividad).")
        uploaded_files = st.file_uploader(
            "Arrastre y suelte los archivos .xlsx aquí",
            type=['xlsx'],
            accept_multiple_files=True
        )
        if uploaded_files:
            # Botón de carga y procesamiento (solo se ejecuta si hay archivos y no se ha cargado)
            if st.button("Procesar Archivos Cargados"):
                st.session_state['dfs'] = load_data_from_upload(uploaded_files)
                st.session_state['data_loaded'] = True
                st.success("¡Datos cargados y pre-procesados con éxito!")

    # Placeholder de Conexión SQL
    st.sidebar.markdown("---")
    st.sidebar.subheader("Conexión a Datos Vivos (Simulado)")
    st.sidebar.success("Estado: Conectado a SQL DB (Simulación)")

    st.sidebar.markdown("---")
    st.sidebar.subheader("Navegación por Módulos")

    # Selector de módulo en el sidebar
    selected_module = st.sidebar.radio("Selecciona un Módulo", MODULOS)

    # Slider de Horizonte de Proyección para Módulos Predictivos
    st.sidebar.markdown("---")
    ...
    # Contenido central basado en el módulo seleccionado
    if not st.session_state['data_loaded']:
        st.title("Bienvenido a Horizonte Empresarial 360")
        st.info("Por favor, cargue los archivos de datos en la sección 'Cargar Datos' del menú lateral para comenzar a navegar por los módulos.")
    else:
        ...
        elif selected_module == "Módulo Gobernanza y Dueños":
            display_gobernanza_module(dfs)

        elif selected_module == "Módulo Financiero":
            display_financiero_module(dfs['df_financiero'], st.session_state['HORIZONTE_PROYECCION'], st.session_state['NUM_SIMULACIONES'])

        elif selected_module == "Módulo Sostenibilidad Social RRHH":
            display_rrhh_module(dfs['df_rrhh'])

        elif selected_module == "Módulo Sostenibilidad Ambiental":
            display_sostenibilidad_ambiental_module(dfs['df_sostenibilidad'])

        elif selected_module == "Módulo Riesgos HSEQ":
            display_riesgos_hseq_module(dfs['df_riesgos'])

        elif selected_module == "Módulo Comercial":
            display_comercial_module(dfs['df_comercial'])

        elif selected_module == "Módulo Operativo (Productividad)":
