...
def display_gobernanza_module(all_dfs: Dict[str, pd.DataFrame]):
    ...
    # 1. Indicadores Clave de Desempeño (KPIs Consolidado)
    ...
    if not df_financiero.empty and not df_sostenibilidad.empty and not df_riesgos.empty:
        ...
        st.warning("No hay datos cargados para generar el Health Score. Asegúrese de cargar todos los archivos requeridos.")
...
def display_financiero_module(df_financiero: pd.DataFrame, HORIZONTE_PROYECCION: int, NUM_SIMULACIONES: int):
    ...
    with col_hist2:
        st.metric("Efectivo Final (Último Periodo)", f"COP {df_financiero['Efectivo_Final'].iloc[-2]:,.0f}") # -2 porque el último valor es el de inicio del siguiente
    ...
    effective_start_cash = df_financiero['Efectivo_Final'].iloc[-2] # Efectivo final del último mes registrado
...
# --- Módulo de Sostenibilidad Social RRHH (Mantener funcionalidad original) ---
...
def display_sostenibilidad_ambiental_module(df_sostenibilidad: pd.DataFrame):
    ...
    co2_alcance = pd.DataFrame({
        'Alcance': ['Alcance 1 (Combustible)', 'Alcance 2 (Electricidad)'] sensible a mayúsculas/minúsculas)
...
# --- Módulo Comercial (Mantener funcionalidad original) ---
...
# --- Módulo Operativo (Productividad/Procesos) (Mantener funcionalidad original) ---
